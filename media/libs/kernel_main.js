...
BX.browser = {
	...
	addGlobalClass: function() {
		...
		{
			globalClass += " bx-ie bx-ie8" + (!BX.browser.IsDoctype() ? " bx-quirks" : "");
		}
		else if (/MSIE 9/.test(navigator.userAgent))
		{
			globalClass += " bx-ie bx-ie9" + (!BX.browser.IsDoctype() ? " bx-quirks" : "");
		}
		else if (/MSIE 10/.test(navigator.userAgent))
		{
			// it seems IE10 doesn't have any specific bugs like others event in quirks mode
			globalClass += " bx-ie bx-ie10";
		}
		...
		var onreadystatechange = config.xhr.onreadystatechange = function(additional)
		{
			...
			else
			{
				if (config.xhr.readyState == 4 || additional == 'run')
				{
					...
					if (status == 'success')
					{
						var authHeader = !!config.skipAuthCheck
							? false
